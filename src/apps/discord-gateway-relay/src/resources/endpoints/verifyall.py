...
class VerifyallEndpoint(RelayEndpoint):
    ...
    async def handle(self, request: RelayRequest):
        ...
        # chunk will be sent.
        asyncio.create_task(self._handle_chunks(
            guild, channel_id, chunk_limit))

    async def _handle_chunks(self, guild: Guild, channel_id, chunk_limit):
        members = guild.members
        if not guild.chunked:
            logging.debug(f"Chunking the members of the guild.")
        ...
        logging.debug(
            f"Splitting the chunked guild members into smaller chunks.")
        split_chunk = [members[i: i + chunk_limit]
                       for i in range(0, len(members), chunk_limit)]
        ...
        for i in range(len(split_chunk)):
            ...
            if response.status > 400:
                logging.error(
                    f"BOT SERVER RESPONSE: {response.status}, {text}")
                break
