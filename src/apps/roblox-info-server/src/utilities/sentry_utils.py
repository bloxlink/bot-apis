...
def trace(op: Optional[str] = None, description: Union[Optional[str], Callable[[Tuple[any], Mapping[str, any]], str]] = None):
    ...
    def inner(func, *args, **kwargs):
        async def wrapper(*inner_args, **inner_kwargs):
            inner_op = working_op
            inner_description = working_description(*inner_args, **inner_kwargs) if callable(working_description) else working_description # TODO: qualname_from_function does not exist #or qualname_from_function(func)
